...
builder.Services.AddTransient<IEmbeddingService>(sp =>
{
    ...
    if (settings.EmbeddingProvider.Equals("OpenAI", StringComparison.OrdinalIgnoreCase))
        return sp.GetRequiredService<OpenAiEmbeddingService>();
    if (settings.EmbeddingProvider.Equals("Jina", StringComparison.OrdinalIgnoreCase))
        return sp.GetRequiredService<JinaEmbeddingService>();
    return sp.GetRequiredService<OllamaEmbeddingService>();
});
...
builder.Services.AddTransient<ILlmService>(sp =>
{
    ...
    if (settings.LlmProvider.Equals("OpenAI", StringComparison.OrdinalIgnoreCase))
        return sp.GetRequiredService<OpenAiLlmService>();
    if (settings.LlmProvider.Equals("Groq", StringComparison.OrdinalIgnoreCase))
        return sp.GetRequiredService<GroqLlmService>();
    return sp.GetRequiredService<OllamaLlmService>();
});

// Configure CORS to allow the separate frontend to call the backend
builder.Services.AddCors(options =>
{
    options.AddPolicy("AllowFrontend", policy =>
    {
        policy
            .AllowAnyOrigin()   // Allow any origin for development (frontend served via file:// or a local server)
            .AllowAnyMethod()
            .AllowAnyHeader();
    });
});
